var jquery3 = jquery3 || $;
var delayValueForFRA = 300;
var defautDelayValue = 300;
...
(function( $ ) {
    ...
    $.fn.dqe = function(options) {
        var myDQE = this;
        myDQE.callbackFunctions = options.callbackFunctions;
        ...
        myDQE.oauth         = settings.oauth2        ? settings.oauth2        : false;
        ...
        //Début changement DECADE
        myDQE.autocompleteInternalFunction = (settings.autocompleteInternalFunction) ? settings.autocompleteInternalFunction : function(){};
        //Fin changement DECADE
            ...
            if (myDQE.server == 'cors') {
                ...
                myDQE.ajax = function(url, callback) {
                    $.ajax({
                        ...
                        error: function(){
                            myDQE.autocompleteInternalFunction();
                        }
                    });
                };
            }
        ...
        myDQE.show_streets = function (reloading, defaultInseeCode,inseeCode) {
            if (inseeCode) {
                myDQE.current_city_id =  inseeCode;
                myDQE.adr_autocomplete_used = true;
            }
           else if(defaultInseeCode){
                myDQE.current_city_id =  defaultInseeCode;
                myDQE.adr_autocomplete_used = true;
            }
            else if (myDQE.one_city){
                myDQE.adr_autocomplete_used = true;
            }
            ...
            if (myDQE.street) {
                ...
                myDQE.street.autocomplete({
                    create: function () {
                        ...
                        $(this).attr("autocomplete", "new-street");
                    },
                    delay: 300,
                    source: function (request, response) {
                        if (single) {
                            ...
                            //Début changement DECADE
                            var requestedAddress = request.term.replace("'", " ");
                            //Fin changement DECADE
                            var params = {
                                fn: 'ADR',
                                IDLocalite: myDQE.current_city_id,
                                Adresse: requestedAddress,
                                Instance: 0,
                                Pays: country
                            };
        ...
        //mode entonnoir
        //mode entonnoir
        myDQE.show_cities = function (target) {
            if (target === 'city') {
                myDQE.clearSection("cityAutocomplete");
            } else {
                myDQE.clearSection("city");
            }
            myDQE.override(true, "zip"); //Evènement lancé au onInput du champ zip
            if (myDQE.zip.val() != 0) {
                var input = myDQE.zip.val();
            } else {
                var input = myDQE.city.val();
            }
        ...
        myDQE.activate_zipcity_autocomplete = function() {
            ...
            if (myDQE.zipcity && !myDQE.single) {
                myDQE.removeAutocomplete(myDQE.zipcity);
                myDQE.zipcity.add(myDQE.city).autocomplete({
                    create: function() {
                        $(this).data('ui-autocomplete')._renderItem = myDQE.render_item;
                    },
                    delay: 300,
                    ...
                    select: function(event, ui) {
                        ...
                        if(myDQE.zip && myDQE.city){
                            ...
                            myDQE.zipcity.val(ui.item.label);
                            if (document.getElementById('city'))  $('#city').val(ui.item.city);
                            if (document.getElementById('zip'))  $('#zip').val(ui.item.zip);
                        }
                        myDQE.selected_zipcity_value = myDQE.zipcity.val();
                        if (myDQE.local && ui.item.local) {
                            myDQE.local.val(ui.item.local);
                        ...
                        myDQE.current_zip = ui.item.zip;
                        myDQE.current_city = ui.item.city;
                        myDQE.show_streets('','',ui.item.city_id);
                        myDQE.execute_trigger('zip', [ui.item.value, ui.item.zip]);
                        myDQE.execute_trigger('city', [ui.item.value, ui.item.city]);
        ...
        myDQE.setGeolocCity = function(pos) {
            ...
            myDQE.ajax(url, function(data) {
                if (!data['1']) {
                    myDQE.callbackFunctions.errorCallBackFunction();
                    return;
                }
                data = data['1'];
                if (myDQE.zipcity) {
                ...
                //Début Modification Decade
                if (myDQE.insee) myDQE.insee.val(data.IDLocalite);
                myDQE.callbackFunctions.setGeolocCityCallBackFunction();
                //Fin Modification Decade
        ...
        myDQE.activate_single_autocomplete = function() {
            ...
            if (myDQE.single) {
                ...
                myDQE.single.autocomplete({
                    ...
                    source: function(request, response) {
                        var country = myDQE.selected_country();
                        var url = "https://prod2.dqe-software.com/SINGLEV2/?Pays=" + country + "&Adresse=" + encodeURIComponent(request.term);
                        if (myDQE.selected_country() == 'GBR') url = "http://37.187.165.132:18513/SINGLE/?Pays=" + country + "&Adresse=" + encodeURIComponent(request.term);

                        if (myDQE.lat || myDQE.lon) url += "&lat=" + myDQE.lat + "&lon=" + myDQE.lon;
                        if (myDQE.street_type && myDQE.country == 'FRA') url += "&Version=1";
                        url += "&Licence=" + myDQE.license;
                        myDQE.ajax(url, function(data) {
                    ...
                    select: function(event, ui) {
                        if (myDQE.street || myDQE.city ||myDQE.zip|| myDQE.prov || myDQE.local) {
                            ...
                            if (myDQE.zipcity) {
                                myDQE.zipcity.val(ui.item.zip + " " + ui.item.city);
                                myDQE.selected_zipcity_value = myDQE.zipcity.val();
                            }
         ...
         */
        myDQE.check = function(callback_function_name, address, addressType) {
            ...
            myDQE.ajax(url, function(data) {
                ...
                callback_function_name(responses, addressType);
            });
        };
        ...
        myDQE.url = function(data) {
            var host = "https://prod2.dqe-software.com";
            var parameters = [];
            for (var key in data) {
            ...
            if (myDQE.oauth == 1 && myDQE.token !== "") {
                myDQE.auth('dqe');
                parameters.push("token" + "=" + myDQE.token);
            }
            ...
            return host + '/' + data["fn"] + "/?" + parameters.join("&") + "&Licence=" + myDQE.license;
        };


        myDQE.auth = function (client_secret) {
            var url = "https://prod2.dqe-software.com/oauth/ACCESS_TOKEN/?client_secret="+ client_secret + "&grant_type=" + myDQE.grant_type + "&licence=" + myDQE.license ? myDQE.license : '';
            myDQE.ajax(url, function(data) {
                $(document).trigger('Dqe_auth', [data.access_token]);
                myDQE.token = data.access_token;
                setTimeout(myDQE.auth, 280000, client_secret);
            });
        };

        [$(settings.city), $(settings.zip), $(settings.street), $(settings.street_type), $(settings.number), $(settings.compl), $(settings.local), $(settings.prov), $(settings.zipcity), $(settings.company), $(settings.single), $(settings.insee)].forEach( function (field) {
            field.on('focus', function () {
                if (myDQE.token === "") myDQE.auth('dqe');
            });
        });

        if (myDQE.zip && myDQE.city && !myDQE.single) {
            var inputEventFunction = function (e) {
                if (myDQE.zip_pattern.hasOwnProperty(myDQE.selected_country())) {
                    myDQE.show_cities(e.data.target);
                }
            };
            myDQE.zip.on("input", null, {target: "zip"}, inputEventFunction);
            myDQE.city.on("input", null, {target: "city"}, inputEventFunction);
        }
